...
DisplayIntro::DisplayIntro(QGraphicsScene *scene, ThemeManager *theme, QGraphicsView *parent)
    : Themeable(QStringLiteral("introdisplay"), theme)
    , QObject(parent)
{
    ...
    mStartButton[0]->show();
    connect(mStartButton[0]->notify(), &SpriteNotify::signalNotify, this, &DisplayIntro::buttonPressed);
    if (Prefs::startcolouryellow())
        mStartButton[0]->setStatus(true);

    mStartButton[1] = new ButtonSprite(false, QStringLiteral("button1_start"), mTheme, 1, mScene);
    mSprites.append(mStartButton[1]);
    mStartButton[1]->show();
    connect(mStartButton[1]->notify(), &SpriteNotify::signalNotify, this, &DisplayIntro::buttonPressed);
    if (Prefs::startcolourred())
        mStartButton[1]->setStatus(true);
    ...
    connect(mPlayerButton[0]->notify(), &SpriteNotify::signalNotify, this, &DisplayIntro::buttonPressed);
    if (Prefs::input0mouse() || Prefs::input0key())
        mPlayerButton[0]->setStatus(true);
    ...
    connect(mPlayerButton[1]->notify(), &SpriteNotify::signalNotify, this, &DisplayIntro::buttonPressed);
    if ((Prefs::input1mouse() || Prefs::input1key()) && !mPlayerButton[0]->status())
        mPlayerButton[0]->setStatus(true);
...
void DisplayIntro::buttonPressed(QGraphicsItem *item, int id)
{
    ...
    if (no >= 10 && no <= 13 && status == 1) {
        ...
        if (mPlayerButton[0]->status())
            input0 = KGameIO::MouseIO;
        if (mPlayerButton[1]->status())
            input1 = KGameIO::MouseIO;
...
void DisplayIntro::changeTheme()
{
    ...
    mTextQuicklaunch->setPos(posQuickstart.x() * width, posQuickstart.y() * height + (textHeight - bounding.height()) / 2);
    ...
    mTextStartplayer->setPos(posStartplayer.x() * width, posStartplayer.y() * height + (textHeight - bounding.height()) / 2);
    bounding = mTextColor->boundingRect();
    mTextColor->setPos(posColor.x() * width, posColor.y() * height + (textHeight - bounding.height()) / 2);
...
void DisplayIntro::delaySprites(int duration)
{
    ...
    for (int i = 0; i < mSprites.size(); ++i) {
        ...
        if (mSprites.at(i)->type() != QGraphicsItem::UserType + 1)
            continue;
...
int DisplayIntro::createAnimation(bool restartTime)
{
    ...
    static int moves[] = {3, 4, 4, 3, 3, 4, 4, 1, 3, 5, 3, 3, 6, 6, 6, 6, 6, 0, 6, 4, 5, 5, 2, 2, 5, 5, 1, 4, 5, 0, 0, 1, 1, 1, 2, 2, 2, 2, 1, 0, 0, 0};
    ...
    for (int i = 0; i < mSprites.size(); ++i) {
        ...
        int no = sprite->number();
        {
            ...
            if (sprite->animationDuration() > maxDuration)
                maxDuration = sprite->animationDuration();
        }
    } // end list loop
    // ============================================================================
...
QGraphicsItem *DisplayIntro::findSprite(QPoint pos)
{
    ...
    for (int i = 0; i < mSprites.size(); ++i) {
        ...
        if (item->contains(p) && (!found || (found->zValue() < item->zValue())))
            found = item;
    }
    return found;
}
