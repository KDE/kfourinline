...
void KGameConnectWidget::slotGamesFound()
{
    ...
    if (autoselect && d->mClientName->count())
        slotGameSelected(0);
}
...
{
    if (nr >= (d->mBrowser->services().count()) || nr < 0)
        return;
    if (!d->mHost->isEnabled())
        return; // this is server mode, do not overwrite host and port controls
    KDNSSD::RemoteService::Ptr srv = d->mBrowser->services()[nr];
    if (!srv->isResolved() && !srv->resolve())
        return;
