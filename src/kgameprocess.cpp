...
void KGameProcess::sendSystemMessage(QDataStream &stream, int msgid, quint32 receiver)
{
    ...
    if (mMessageIO)
        ...
        fprintf(stderr, "KGameProcess::sendSystemMessage:: NO IO DEVICE ... WILL FAIL\n");
}
...
// ----------------------- KMessageFilePipe ---------------------------
KMessageFilePipe::KMessageFilePipe(QObject *parent, QFile *readfile, QFile *writefile)
    : KMessageIO(parent)
{
    mReadFile = readfile;
...
void KMessageFilePipe::exec()
{
    ...
    while (mReceiveCount >= mReceiveBuffer.size())
