...
DisplayGame::DisplayGame(ReflectionGraphicsScene *scene, ThemeManager *theme, QGraphicsView *parent)
    : Themeable(QStringLiteral("gamedisplay"), theme)
    , QObject(parent)
{
    ...
    for (int i = 0; i < deco.size(); i++) {
        ...
        if (!sprite)
            qCCritical(KFOURINLINE_LOG) << "Cannot load sprite" << deco.at(i);
        mSprites.append(sprite);
        sprite->show();
    }
...
void DisplayGame::changeTheme()
{
    ...
    for (int i = 0; i < 7; i++) {
        ...
        QPointF to = QPointF(board_spread.x() * i + arrow_pos.x(), board_spread.y() * 0 + arrow_pos.y());
    ...
    for (int i = 0; i < 42; i++) {
        ...
        QPointF to = QPointF(board_spread.x() * x + board_pos.x(), board_spread.y() * y + board_pos.y());
    ...
    if (config.readEntry("use-reflection", false)) {
        dynamic_cast<KWin4View *>(mView)->setReflection((int)pos.x(),
                                                        (int)(pos.y() + pos.height()),
                                                        (int)(mScene->sceneRect().width() - pos.x()),
                                                        (int)(pos.height() * 0.3));
    } else {
...
void DisplayGame::displayArrow(int x, int color)
{
    ...
    if (color == Yellow)
        mArrows.value(x)->setFrame(1);
    else
        mArrows.value(x)->setFrame(2);
}
...
SpriteNotify *DisplayGame::displayPiece(int x, int y, int color, int no, bool animation)
{
    ...
    if (animation) {
        QPointF from = QPointF(board_spread.x() * x + board_pos.x(), board_spread.y() * (-1) + board_pos.y());
        QPointF to = QPointF(board_spread.x() * x + board_pos.x(), board_spread.y() * y + board_pos.y());
...
void DisplayGame::displayStar(int x, int y, int no)
{
    ...
    QPointF pos = QPointF(board_spread.x() * x + board_pos.x(), board_spread.y() * y + board_pos.y());
